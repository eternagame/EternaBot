...
def generate_plot(designs, strategies, ytics, ylabels, name):
	...
	for strategy in strategies:
		...
		for ii in range(0, len(designs)):
			design = designs[ii]
			prediction = strategy.score(design,strategy.default_params_)
			full_scores.append(prediction)
			if(prediction > eterna_utils.UNSCORABLE):
				scores.append([prediction, 1])
				nature_scores.append([design['score']])
			
		nature_matrix = numpy.matrix(nature_scores)
		prediction_matrix = numpy.matrix(scores)
		X = scipy.linalg.lstsq(prediction_matrix, nature_matrix)
			
		for ii in range(0, len(designs)):
			design = designs[ii]
			normalized_scores.append(full_scores[ii] * X[0][0][0] + X[0][1][0])
			
		for ii in range(0, len(designs)):
			design = designs[ii]
			plotout.write("%d %d %d\n" % (strategy_walker, ii, normalized_scores[ii]))
		
		plotout.write("\n");	
	
		for ii in range(0, len(designs)):
			design = designs[ii]
			plotout.write("%d %d %d\n" % (strategy_walker+1, ii, normalized_scores[ii]))
	...
	plotdem_dis.write("set zrange [0: 100]\n")
	
	plotdem_con.write("set grid lt 1 lc rgb '#000000'\n")
	plotdem_con.write("set grid xtics\n")
	plotdem_con.write("set zrange [0: 100]\n")
	
	plotdem_dis.write('set palette model RGB defined (0 "black", 5 "black", 5 "blue", 6 "blue", 6 "green", 7 "green", 7 "orange", 8 "orange", 8 "red", 9 "red", 9 "dark-red")\n')
	plotdem_con.write('set palette model RGB defined (0 "black", 9 "black", 11 "blue", 13 "green", 15 "orange", 17 "red", 18 "red")\n')
...
generate_plot(designs,strategies,ytics,ylabels,"puzzlesort")		
designs = sorted(designs, key=lambda design: (design['score']), reverse=False)
generate_plot(designs,strategies,[],[],"globalsort")
...
for ii in range(0,len(ytics)):
	...
	generate_plot(designs,strategies,ytics,ylabels,"puzzlesort_%s" % title)
