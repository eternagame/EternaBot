...
def calculate_average_squared_error(params, input_designs,strategy): 
	...
	for ii in i_range:
		...
		if(prediction <= switch_utils.UNSCORABLE):
			continue
...
def calculate_kt_corr(params, input_designs, strategy):
	...
	for ii in i_range:
		...
		for jj in j_range:
			...
			if(input_designs[ii]['predicted_score'] <= switch_utils.UNSCORABLE or input_designs[jj]['predicted_score'] <= switch_utils.UNSCORABLE):
				pred_comp = 0
...
def get_score_plot_code(designs,strategy):
	...
	ret += ('<script src="https://ajax.googleapis.com/ajax/libs/jquery/2.1.4/jquery.min.js"></script>\n')
	ret += ('<script language="javascript" type="text/javascript" src="../../jquery.flot.min.js"></script>\n\n')
	...
	for ii in range(0, len(designs)):
		...
		if(designs[ii]['predicted_score'] > switch_utils.UNSCORABLE and designs[ii]['score'] > 20): 
