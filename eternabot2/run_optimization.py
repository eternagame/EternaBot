...
def run_all():
    designs = switch_utils.get_switch_designs_from_csv("training.csv")
    ...
    for fname in dirList:
        if re.search('\.py$', fname):
            ...
            run_strategy(strategy, designs, "strategy")

def run_strategy(strategy,designs,prefix):
    ...
    file_prefix = ""
    f_output = open("./" + prefix + "/outputs/" + file_prefix + strategy.author_ + "'s " + strategy.title_ + ".html","wb")
    ...
    for ii in i_range: 
        ...
        body_data += str(designs[ii]['soltitle'])
        # + " (id:" + str(designs[ii]['solnid']) + ")")
        # body_data += ("</a>")
        body_data += "</td>"
        body_data += ("<td style='width:50px'>")
        if(designs[ii]['predicted_score'] > switch_utils.UNSCORABLE):
    ...
    f_output.write("Thank you for sharing your idea, and we look forward to other brilliant strategies from you!");
