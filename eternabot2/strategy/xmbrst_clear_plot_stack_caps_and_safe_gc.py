...
class Strategy(strategy_template.Strategy):
	def __init__(self):
		...
		self.default_params_ = [1.0, 1.0, 2.0, 0.8, 0.05, 0]
	...
	def score(self, design, params):			
		...
		for ntss in states:
			...
			npairs = design[nts]['default']['gc'] + design[nts]['default']['gu'] + design[nts]['default']['ua']
			dotplot = design[nts]['default']['dotplotU']
			pairmap = design[nts]['default']['pairmap']
			elements = design[nts]['default']['secstruct_elements']
			...
			gc_penalty = 0
			if(npairs > 0):
				if(float(design[nts]['default']['gc'])/npairs > params[3]):
					gc_penalty = 1
			...
			score = (2.0 + cap_score * params[1] + plotscore * params[0] - gc_penalty * params[2]) * 25;
			totalscore += score
