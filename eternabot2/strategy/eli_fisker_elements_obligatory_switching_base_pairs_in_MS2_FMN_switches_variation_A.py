...
class Strategy(strategy_template.Strategy):
    def __init__(self):
        ...
        # Note: This strategy is variation A (Eli's original description)
        self.title_ = ("[Strategy Market] [Switch]"
                       "Elements: Obligatory switching base pairs in"
                       "MS2 FMN switches varA")
        ...
        for n in range(1, 3):  # [1, 2]
            ...
            for i in seq_range:
                if design[state]['default']['secstruct'][i] == '(':
                    # base paired
                    pair1 = design[state]['default']['pairmap'][i]
                    pair2 = design['state' + otherstate]['default']['pairmap'][i]
                    if pair1 != pair2:
                        switchbp += 1
