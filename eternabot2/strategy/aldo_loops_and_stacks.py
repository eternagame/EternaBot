...
class Strategy(strategy_template.Strategy):
	def __init__(self):
		...
		self.default_params_ = [2,1,1,2,1,1,0.5, 0.55, 100, 0.05, 0]
	...
	def score(self, design, params):
		...
		for state in states:
			...
			modifier = 1.0 - abs(float(design[n]['default']['gc'])/total_pairs - params[7] )
			
			#return score				
			score = params[8] * modifier * (float(score)) / len(sequence)

			totalscore += score

		totalscore = float(totalscore) / len(states)
		return (params[9] * totalscore) + params[10]
