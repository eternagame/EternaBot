...
class Strategy(strategy_template.Strategy):
    def __init__(self):
        ...
        self.default_params_ = [100, -1, -2, 0.05, 0] # [Score for Good GC multiloop, Amount to modify good score if a GC pair is wrong direction, Score for Non-GC pair in multiloop]
    ...
    def score(self, design, params):
        ...
        for n in states:
            pairmap = design['state' + str(n)]['default']['pairmap']
            ...
            for ii in range(len(pairmap)):
                ...
                gc_multiloop_string = self.get_gc_multiloop_members(ii, design, 'state' + str(n))
            ...
            totalscore += score # Returned if there are no multiloops with GC pairs only

        totalscore = float(totalscore) / len(states)
        return (params[3] * totalscore) + params[4]
    ...
    def get_gc_multiloop_members(self, index, design, pre):
        returnstring = ""
        sequence = design['sequence']
        pairmap = design[pre]['default']['pairmap']
