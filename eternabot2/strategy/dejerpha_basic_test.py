...
class Strategy(strategy_template.Strategy):
	def __init__(self):
		...
		self.default_params_ = [0.5, 100, -1.5, 1, 1, 77, 97, 0.05, 0]
	...
	def score(self, design, params):
		...
		for state in states:
			...
			mp = design[n]['default']['meltpoint']
			
			total_pairs = design[n]['default']['gc'] + design[n]['default']['gc'] + design[n]['default']['ua']
			
			score = 100
			
			if(total_pairs > 0):
				score -= abs(float(design[n]['default']['ua'])/total_pairs - params[0]) * params[1] 
		...
		return (totalscore * params[7]) + params[8]
