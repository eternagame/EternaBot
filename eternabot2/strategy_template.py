...
class Strategy(json.JSONEncoder):
	...
	def load_opt_params(self):
		...
		params_name = self.file_prefix_ + "/params/" + self.author_ + "_" + self.title_ + ".params"
		try :
	...
	def save_cv_kt_score(self, n, ktscore):
		score_name = self.file_prefix_ + "/scores/" + self.author_ + "_" + self.title_ + ".cvkt" + str(n)
	...
	def get_normalization(self, params):
		...
		fm = open( self.file_prefix_ + "/normalized/" + strategy_name + "_mean.txt", "r");
		fs = open( self.file_prefix_ + "/normalized/" + strategy_name + "_stdev.txt", "r");
	...
	def set_normalization(self, designs, params):
		...
		fm = open(self.file_prefix_ + "/normalized/" + strategy_name + "_mean.txt", "w")
		fs = open(self.file_prefix_ + "/normalized/" + strategy_name + "_stdev.txt", "w")
